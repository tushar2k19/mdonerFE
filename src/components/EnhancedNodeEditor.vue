  ...
  methods: {
    ...
    buildHierarchy(flatNodes) {
      ...
      const byPositionThenId = (a, b) => {
        const pa = a.position != null ? Number(a.position) : 0
        const pb = b.position != null ? Number(b.position) : 0
        if (pa !== pb) return pa - pb
        return mapKey(a.id).localeCompare(mapKey(b.id), undefined, { numeric: true })
      }

      const nodeMap = new Map()
      flatNodes.forEach(node => {
        const idKey = mapKey(node.id)
        if (idKey == null || nodeMap.has(idKey)) return
        nodeMap.set(idKey, { ...node, children: [] })
      })

      const linkChildren = (enhanced) => {
        const parentKey = mapKey(enhanced.id)
        const sources = flatNodes
          .filter(n => mapKey(n.parent_id) === parentKey && nodeMap.has(mapKey(n.id)))
          .sort(byPositionThenId)
        const seen = new Set()
        enhanced.children = []
        sources.forEach(src => {
          const ck = mapKey(src.id)
          if (ck == null || seen.has(ck)) return
          seen.add(ck)
          const child = nodeMap.get(ck)
          if (child) enhanced.children.push(child)
        })
        enhanced.children.forEach(linkChildren)
      }

      const rootSources = flatNodes
        .filter(n => {
          const pk = mapKey(n.parent_id)
          return !pk || !nodeMap.has(pk)
        })
        .sort(byPositionThenId)
      ...
      rootSources.forEach(src => {
        const k = mapKey(src.id)
        if (k == null || seenRoots.has(k)) return
        seenRoots.add(k)
        const node = nodeMap.get(k)
        if (node) rootNodes.push(node)
      })

      rootNodes.forEach(linkChildren)
      return rootNodes
    },
