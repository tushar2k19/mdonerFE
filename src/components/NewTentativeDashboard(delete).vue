<template>
  <div class="dashboard-container">
    <!-- Meeting draft controls (NewTentativeDashboard = meeting-centric only) -->
    <div class="meeting-dashboard-bar" role="region" aria-label="Meeting draft controls">
      <div class="meeting-bar-row meeting-bar-row-main">
        <label class="meeting-bar-field">
          <span class="meeting-bar-label">Target meeting date</span>
          <input
        ...
        <div class="meeting-bar-hint">
          Draft from <code>/meeting_dashboard/draft</code>.
      ...
      <div v-if="userRole === 'editor'" class="meeting-bar-row meeting-bar-actions">
        ...
        >
          {{ meetingPublishSaving ? 'Publishing…' : 'Publish dashboard' }}
        </button>
        <button
          type="button"
    ...
    <div class="global-action-menu" 
      ...
      <template v-if="featureMeetingDashboard">
        ...
        <button class="menu-item" @click="forceHideMenu()">Assignments (Coming Soon)</button>
        <button class="menu-item" @click="forceHideMenu()">Live Assignment Status</button>
        <button @click="openTagsModal(getCurrentTask()); forceHideMenu()" class="menu-item tags">Tags</button>
      </template>
    </div>
    ...
    <div v-if="showResetConfirmModal" class="meeting-reset-modal-overlay" @click.self="showResetConfirmModal = false">
      <div class="meeting-reset-modal" role="dialog" aria-modal="true" aria-labelledby="meeting-reset-title">
        <h3 id="meeting-reset-title" class="meeting-reset-modal-title">Reset draft?</h3>
        <p class="meeting-reset-modal-text">This reloads the living draft from the last published meeting snapshot. Unpublished edits may be lost.</p>
...
export default {
  ...
  data () {
    return {
      ...
      targetMeetingDateSaveTimer: null,
  ...
  async created () {
    ...
    await this.loadMeetingDraftSettings()
  ...
  beforeDestroy() {
    ...
    if (this.targetMeetingDateSaveTimer) {
      clearTimeout(this.targetMeetingDateSaveTimer)
      this.targetMeetingDateSaveTimer = null
    }
  ...
  methods: {
    ...
    async fetchTasksByDate() {
      try {
        ...
        const response = await this.$http.secured.get('/meeting_dashboard/draft', { params })
        ...
        if (response.data.draft_settings && response.data.draft_settings.target_meeting_date) {
          ...
          if (synced && !this.targetMeetingDate) {
    ...
    async loadMeetingDraftSettings () {
      try {
        ...
        if (data && data.target_meeting_date) {
          this.targetMeetingDate = this.toDateInputValue(data.target_meeting_date)
        }
      } catch (e) {
    ...
    onTargetMeetingDateChange () {
      if (this.targetMeetingDateSaveTimer) {
        clearTimeout(this.targetMeetingDateSaveTimer)
      }
      this.targetMeetingDateSaveTimer = setTimeout(() => {
        this.persistTargetMeetingDate()
      }, 400)
    },
    ...
    async publishMeetingDashboard () {
      ...
      try {
        ...
        this.$toast.success('Dashboard published.')
        await this.fetchTasksByDate()
      } catch (e) {
    ...
    async confirmResetDraft () {
      ...
      try {
        ...
        this.$toast.success('Draft reset to published.')
        await this.fetchTasksByDate()
      } catch (e) {
...
.meeting-reset-confirm:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
.menu-item-disabled {
