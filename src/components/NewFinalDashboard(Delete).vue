<template>
  <div class="dashboard-container" :class="{ 'new-final-meeting-readonly': featureMeetingDashboard }">
    <div v-if="featureMeetingDashboard" class="meeting-published-bar" role="region" aria-label="Published meeting dashboard">
      <label class="meeting-published-field">
        <span class="meeting-published-label">Meeting date</span>
        <select
          v-if="meetingDateOptions.length"
          v-model="selectedMeetingDate"
          class="meeting-published-select"
          @change="fetchApprovedTasks"
        >
          <option v-for="opt in meetingDateOptions" :key="String(opt.meeting_date)" :value="meetingDateToInput(opt.meeting_date)">
            {{ meetingDateToInput(opt.meeting_date) }}
          </option>
        </select>
        <input
          v-else
          v-model="selectedMeetingDate"
          type="date"
          class="meeting-published-date-input"
          @change="fetchApprovedTasks"
        >
      </label>
      <div v-if="publishedMeta && publishedMeta.published_at" class="meeting-published-meta">
        Published {{ formatPublishedAt(publishedMeta.published_at) }}
        <span v-if="publishedMeta.target_meeting_date"> · Target {{ meetingDateToInput(publishedMeta.target_meeting_date) }}</span>
      </div>
      <div v-else-if="publishedLoadEmpty" class="meeting-published-meta muted">No snapshot for this meeting date.</div>
    </div>
...
export default {
  ...
  computed: {
    emptyStateTitle () {
      if (this.searchQuery.length > 0) return 'No Search Results'
      if (this.featureMeetingDashboard) return 'No Published Dashboard'
      return 'No Approved Tasks'
    },
    emptyStateSubtitle () {
      if (this.searchQuery.length > 0) {
        return this.featureMeetingDashboard
          ? 'No published tasks match your search.'
          : 'No approved tasks match your search criteria.'
      }
      if (this.featureMeetingDashboard) {
        return 'No published pack for this meeting date. Publish from the editor dashboard or pick another date.'
      }
      return 'No approved tasks found for the selected date.'
    },
    statusClass() {
      return {
        draft: 'status-draft',
  ...
  methods: {
    ...
    async loadMeetingDates () {
      try {
        ...
        this.meetingDateOptions = Array.isArray(data.meeting_dates) ? data.meeting_dates : []
        if (this.meetingDateOptions.length && !this.selectedMeetingDate) {
          this.selectedMeetingDate = this.meetingDateToInput(this.meetingDateOptions[0].meeting_date)
        }
        if (!this.selectedMeetingDate) {
          this.selectedMeetingDate = new Date().toISOString().split('T')[0]
        }
      } catch (e) {
        console.warn('Could not load meeting dates:', e)
        if (!this.selectedMeetingDate) {
          this.selectedMeetingDate = new Date().toISOString().split('T')[0]
        }
      }
    ...
    async fetchApprovedTasks() {
      try {
        ...
        const meetingDateParam = this.selectedMeetingDate || new Date().toISOString().split('T')[0]
        ...
        this.publishedMeta = {
          meeting_date: response.data.meeting_date,
          published_at: response.data.published_at,
          target_meeting_date: response.data.target_meeting_date
        }
        this.publishedLoadEmpty = !!(response.data.empty || (response.data.tasks && response.data.tasks.length === 0))
...
.meeting-published-field {
  display: inline-flex;
  flex-direction: column;
  gap: 0.25rem;
}
.meeting-published-label {
  font-weight: 600;
  color: #065f46;
}
.meeting-published-select,
.meeting-published-date-input {
  padding: 0.35rem 0.5rem;
  border-radius: 6px;
  border: 1px solid #34d399;
  font-size: 0.85rem;
}
.meeting-published-meta {
  color: #047857;
  font-size: 0.85rem;
}
.meeting-published-meta.muted {
  color: #6b7280;
}
.new-final-meeting-readonly .action-content-cell {
  user-select: text;
}
